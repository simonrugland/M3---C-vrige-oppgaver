...
namespace Core_og_unit_test
{
    class NumberPuzzleModel
    {
        ...
        public bool IsSolved => Enumerable.Range(0, _numbers.Length)
                                                .All(i => _numbers[i] == i + 1);

        public NumberPuzzleModel()
        {
        ...
        {
            var blankNeighbourIndex = GetBlankNeighboutIndex(index);
        ...
        private int? GetBlankNeighboutIndex(int index)
        {
            var neighbourIndexes = new[]
                {index - 3, index - 1, index + 1, index + 3};
